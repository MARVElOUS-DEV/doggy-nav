    ...
    return request(this.url, async (error, res, body) => {
      if (!error && res.statusCode == 200) {
        ...
        for (let i = 0; i < $cardBlock.length; i++) {
          const secondCategoryName = $('.panel-title.card').eq(i).text()
            .trim();
          ...
          for (let j = 0; j < length; j++) {
            const name = $('.panel').eq(i).find('.card-title')
              ...
              .text()
              .trim();
            const href = $('.panel').eq(i).find('.card .card-heading')
              .eq(j)
              .attr('title');
            const desc = $('.panel').eq(i).find('.card .card-body')
              .eq(j)
              .text()
              .trim();
            const logo = this.rootUrl + $('.panel').eq(i).find('.card-icon img')
              .eq(j)
              .attr('src');
  ...
  return request(url, async (error, res, body) => {
    if (!error && res.statusCode == 200) {
      ...
      for (let i = 0; i < $cardBlock.length; i++) {
        ...
        for (let j = 0; j < length; j++) {
          const name = $aTags.eq(j).find('strong').text()
            .trim();
