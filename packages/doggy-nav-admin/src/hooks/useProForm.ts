...
export default function useProForm(
  props: IProps,
): Pick<IProps, 'visible' | 'onVisibleChange'> & { formRef: any } {
          ...
          selectedData = {
            ...
            audience: {
              ...
              allowRoles: Array.isArray(selectedData?.audience?.allowRoles)
                ? selectedData.audience.allowRoles.map((r: any) =>
                    typeof r === 'string' ? r : r?.id || r,
                  )
                : [],
              allowGroups: Array.isArray(selectedData?.audience?.allowGroups)
                ? selectedData.audience.allowGroups.map((g: any) =>
                    typeof g === 'string' ? g : g?.id || g,
                  )
                : [],
            },
          };
