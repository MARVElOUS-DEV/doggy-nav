...
import { Button, Card, Col, Divider, Drawer, Form, Input, message, Progress, Row, Select, Space, Upload, Switch, Tabs } from 'antd';
...
type RowItem = {
  ...
  tags?: string[];
  logo?: string;
  categoryId?: string; // backend category _id after creation/mapping
...
type CatItem = {
  ...
  createdId?: string; // backend category _id after creation
};
...
function autoDetectMapping(keys: string[]): Mapping {
  const k = (s: string) => keys.find(x => x.toLowerCase() === s) || keys.find(x => x.toLowerCase().includes(s));
  return {
...
function normalizeItem(src: any, map: Mapping, defaults: Partial<RowItem>, parentGuid?: string | null): RowItem {
  const pick = (key?: string) => (key ? src?.[key] : undefined);
  let tags = pick(map.tags) ?? src?.tags;
  if (typeof tags === 'string') tags = tags.split(',').map((s: string) => s.trim()).filter(Boolean);
...
async function saveBatch(rows: RowItem[], concurrency = 5): Promise<RowItem[]> {
  ...
  async function run(row: RowItem) {
    try {
      await request({ url: API_NAV, method: 'POST', data: {
        name: row.name, href: row.href, desc: row.desc, tags: row.tags, logo: row.logo, categoryId: row.categoryId,
      }});
  ...
  await Promise.all(Array.from({ length: Math.min(concurrency, queue.length || 1) }).map(() => next()));
  return rows;
}
...
export default function BookmarksImportPage() {
  ...
  const [subSelections, setSubSelections] = useState<Record<string, React.Key[]>>({});
  const [saving, setSaving] = useState(false);
  const [progress, setProgress] = useState<number>(0);
  const [topParentCategoryId, setTopParentCategoryId] = useState<string | undefined>();
  ...
  const [mapping, setMapping] = useState<Mapping>(() => autoDetectMapping(keys));
  ...
  const applyMapping = useCallback(() => {
    const json = parseJSONSafe(editorValue);
    if (!json) { message.error('JSON 解析失败'); return; }
    ...
    setCats(catList);
    setRows(deDuped);
    message.success(`解析成功：分类 ${catList.length} 个，网址 ${deDuped.length} 条`);
  }, [editorValue, mapping]);
  ...
  const validateRows = (list: RowItem[]) => {
    ...
    const updated = list.map(r => {
      if (!r.name || !r.href || !isValidURL(r.href)) { r.__status = 'skipped'; invalid++; }
      return r;
    });
    if (invalid) message.warning(`有 ${invalid} 条记录缺少必填或 URL 不合法，已标记为跳过`);
    setRows(updated);
  };
  ...
  const onSaveNavs = async (onlyFailed = false, filterGuid?: string) => {
    const targets = rows.filter(r => (
      ...
      r.__status !== 'skipped' && (!onlyFailed ? r.__status !== 'success' : r.__status === 'failed')
    ));
    if (!targets.length) { message.info('没有需要保存的记录'); return; }
    setSaving(true); setProgress(0);
    let done = 0;
    const wrapped = targets.map(r => ({
      ...r,
      __onDone: () => { done++; setProgress(Math.round(done / targets.length * 100)); }
    }));
    // optionally enrich logo lazily at save-time
    const enriched = (wrapped as any as RowItem[]).map(r => {
      if (!r.logo && fetchLogoOnSave && r.href) {
        try { const { hostname } = new URL(r.href); r.logo = getFaviconSrv(hostname); } catch {}
      }
      return r;
    });
  ...
  const saveCategories = async () => {
    ...
    const created = new Map<string, string>(); // guid -> _id
    ...
    setCats([ ...cats ]);
    if (pending.size) {
      message.error(`部分分类未创建成功：剩余 ${pending.size} 个（可能缺少父级）`);
    } else {
      message.success(`分类创建完成：${created.size} 个`);
    }
    // map category ids onto rows by categoryGuid
    if (created.size) {
      const nextRows = rows.map(r => ({ ...r, categoryId: r.categoryGuid ? created.get(r.categoryGuid) : r.categoryId }));
      setRows(nextRows);
    }
  };
  ...
  const columns = useMemo<ProColumns<RowItem>[]>(() => [
    ...
    { title: '状态', dataIndex: '__status', width: 100, valueType: 'select', valueEnum: {
      pending: { text: '待处理' }, success: { text: '成功' }, failed: { text: '失败' }, skipped: { text: '跳过' }
    } },
    { title: '操作', valueType: 'option', width: 100, render: (_, record) => [
      <a key="edit" onClick={() => { setEditingRow(record); setDrawerOpen(true); }}>编辑</a>
    ]},
  ], []);
  ...
  const catColumns = useMemo<ProColumns<CatItem>[]>(() => [
    ...
    { title: '创建后 _id', dataIndex: 'createdId', width: 260 },
  ], []);
      ...
      <Tabs
        ...
        items={[
            ...
            children: (
              <Card title={
                <Space>
                  <span>分类与下属网址预览</span>
                  <span>顶级父级（可选）：</span>
                  <CategorySelect value={topParentCategoryId} onChange={setTopParentCategoryId} />
                  <Button type="primary" onClick={saveCategories}>保存分类</Button>
                  <span>保存时抓取图标</span>
                  <Switch checked={fetchLogoOnSave} onChange={setFetchLogoOnSave} />
                </Space>
              }>
                ...
                <ProTable<CatItem>
                  ...
                  expandable={{
                    expandedRowRender: (record) => {
                      const subNavs = rows.filter(r => r.categoryGuid === record.guid);
                      return (
                        <ProTable<RowItem>
                          size="small"
                          ...
                          rowSelection={{
                            ...
                            onChange: (keys) => setSubSelections(prev => ({ ...prev, [record.guid]: keys }))
                          }}
                          editable={{ type: 'multiple' }}
                          scroll={{ x: 'max-content' }}
                          toolBarRender={() => [
                            <Space key="sub-bulk">
                              <span>批量设置分类(_id)：</span>
                              <CategorySelect onChange={(v: string) => {
                                const ids = new Set((subSelections[record.guid] || []) as string[]);
                                const next = rows.map(r => ids.has(r.href + (r.name || '')) ? { ...r, categoryId: v } : r);
                                setRows(next);
                              }} />
                              <Button onClick={() => validateRows(subNavs)}>校验</Button>
                              <Button type="primary" onClick={() => onSaveNavs(false, record.guid)}>保存此分类下网址</Button>
                              <Button onClick={() => onSaveNavs(true, record.guid)}>重试失败</Button>
                            </Space>
                          ]}
                        />
                      );
                ...
                >
                  <Form
                    ...
                    onFinish={(vals) => {
                      ...
                      const next = rows.map(r => (r.href + (r.name || '') === key)
                        ? {
                            ...r,
                            desc: vals.desc,
                  ...
                  >
                    ...
                    <Form.Item label="分类(_id)" name="categoryId">
                      <CategorySelect />
                    </Form.Item>
                    <Space>
          ...
          },
          {
            ...
            children: (
              <>
                <Card title="书签 JSON 编辑器" extra={
                  <Space>
                    <Upload beforeUpload={handleFile} showUploadList={false} accept=".json">
                      <Button icon={<UploadOutlined />}>加载 JSON 文件</Button>
                    </Upload>
                  ...
                  <Form layout="inline">
                    <Form.Item>
                      <Button type="primary" onClick={applyMapping}>应用并预览</Button>
                    </Form.Item>
                  </Form>
                </Card>
                <Card style={{ marginTop: 16 }} title="字段映射">
                  <Row gutter={12}>
                    {(['name','href','desc','tags','logo','categoryId'] as (keyof Mapping)[]).map(f => (
                      <Col span={12} key={f} style={{ marginBottom: 8 }}>
                        <Space>
                          <span style={{ width: 90, display: 'inline-block' }}>{f}</span>
                          <Select
                            style={{ width: 220 }}
                            value={(mapping as any)[f]}
                            onChange={(v) => setMapping(m => ({ ...m, [f]: v }))}
                            allowClear
                            placeholder="选择源字段"
                    ...
                    {(['typeKey','guidKey','childrenKey'] as (keyof Mapping)[]).map(f => (
                      <Col span={12} key={f} style={{ marginBottom: 8 }}>
                        <Space>
                          <span style={{ width: 90, display: 'inline-block' }}>{f}</span>
                          <Select
