...
import type { EmailSettingsRepository, EmailSettingsUpsertInput } from '../repositories/EmailSettingsRepository';
...
export class EmailSettingsService {
  ...
  async update(input: Partial<EmailSettingsUpsertInput>): Promise<EmailSettings> {
    ...
    for (const k of required) {
      if (!(k in input) || input[k] === undefined || input[k] === null || String(input[k]).trim() === '') {
    ...
    if (!Array.isArray(input.adminEmails)) input.adminEmails = Array.isArray(input.adminEmails) ? input.adminEmails : [];
    if (typeof input.enableNotifications !== 'boolean') input.enableNotifications = !!input.enableNotifications;
