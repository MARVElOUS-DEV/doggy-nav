  ...
  async update(id: string, patch: InviteCodeUpdatePatch): Promise<InviteCode | null> {
    ...
    if (patch.usageLimit !== undefined) {
      ...
      if (patch.usageLimit < current.usedCount) {
        throw Object.assign(new Error('使用次数上限不能小于已使用次数'), {
