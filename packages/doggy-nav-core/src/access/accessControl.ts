...
// Route access control matrix (shared across server and workers)
export const routePermissions: RoutePermission[] = [
  // User authentication routes
  { method: 'POST', path: '/api/auth/register', require: { level: 'public' }, description: 'User registration' },
  { method: 'POST', path: '/api/auth/login', require: { level: 'public' }, description: 'User login' },
  ...
  // Roles & Groups (list endpoints are optional; server enforces stricter in admin source)
  ...
  // Translation routes
  { method: 'POST', path: '/api/translate', require: { level: 'authenticated' }, description: 'translate tools' },

  // Email settings (sysadmin only)
  { method: 'GET',  path: '/api/email-settings', require: { anyRole: ['sysadmin'] }, description: 'Get email settings' },
  { method: 'PUT',  path: '/api/email-settings', require: { anyRole: ['sysadmin'] }, description: 'Update email settings' },
  { method: 'POST', path: '/api/email-settings/test', require: { anyRole: ['sysadmin'] }, description: 'Test email settings' },
  { method: 'GET',  path: '/api/email-settings/health', require: { anyRole: ['sysadmin'] }, description: 'Email service health check' },
];

// Helper: find the permission definition for a method+path
export function getRoutePermission(method: string, path: string): RoutePermission | undefined {
  ...
  return routePermissions.find((p) => {
...
// Helper: check if a user can access a route according to the matrix
export function hasAccess(permission: RoutePermission, user: AccessUserContext | undefined): boolean {
  ...
  const eff = Array.isArray(user.effectiveRoles) && user.effectiveRoles.length > 0
    ? user.effectiveRoles
