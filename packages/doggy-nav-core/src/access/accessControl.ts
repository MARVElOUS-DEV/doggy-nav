...
export function hasAccess(
  ...
  user: AccessUserContext | undefined
): boolean {
  ...
  if (req.anyPermission && req.anyPermission.some((p) => perms.includes(p) || perms.includes('*')))
    return true;
  if (
    req.allPermissions &&
    req.allPermissions.every((p) => perms.includes(p) || perms.includes('*'))
  )
    return true;
  return false;
}
