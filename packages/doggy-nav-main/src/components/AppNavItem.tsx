...
export default function AppNavItem({ data, onHandleNavClick, onHandleNavStar }: AppNavItemProps) {
  ...
  return (
    <Col xs={24} sm={12} md={8} lg={6} className="website-item text-xs mb-5 overflow-hidden cursor-pointer transition-all duration-300 text-gray-500 relative">
      <div ref={ref} className="wrap rounded-md bg-white cursor-pointer shadow-lg relative h-full flex flex-col">
        {getStatusIndicator()}
        ...
        <Link href={`/nav/${data._id}`} className="info block transition-all duration-300 bg-white p-5 flex flex-col justify-start rounded-t-md flex-grow">
  ...
  width={35}
  height={35}
  className="logo min-w-[35px] w-[35px] h-[35px] rounded-full mr-4 flex-shrink-0"
...
/>
            <div className="info-header-right flex flex-col">
              ...
              <div className="desc mt-1 text-xs truncate">{data.desc || '这个网站什么描述也没有...'}</div>
            </div>
          </div>
        </Link>
        <div className="website-item__footer border-t border-gray-100 bg-white p-2.5 text-right flex flex-shrink-0 rounded-b-md">
