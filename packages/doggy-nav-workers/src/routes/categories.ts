...
categoryRoutes.post('/', createAuthMiddleware({ required: true }), requireRole('sysadmin'), async (c) => {
  try {
    const body = await c.req.json();
    const { name, categoryId, description, icon, showInMenu, onlyFolder, audience } = body || {};
    if (!name || !categoryId) return c.json(responses.badRequest('name and categoryId are required'), 400);
    ...
    await db.prepare(`INSERT INTO categories (id, name, category_id, description, create_at, only_folder, icon, show_in_menu, audience_visibility)
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .bind(id, String(name), String(categoryId), description || '', createAt, onlyFolder ? 1 : 0, icon || '', showInMenu ? 1 : 0, vis)
      .run();
    ...
    if (vis === 'restricted') {
      ...
      for (const rid of allowRoles) {
        await db.prepare(`INSERT INTO category_role_permissions (category_id, role_id) VALUES (?, ?)`)
          .bind(id, rid)
          .run();
      }
      for (const gid of allowGroups) {
        await db.prepare(`INSERT INTO category_group_permissions (category_id, group_id) VALUES (?, ?)`)
          .bind(id, gid)
          .run();
      }
    }
...
categoryRoutes.put('/', createAuthMiddleware({ required: true }), requireRole('sysadmin'), async (c) => {
  try {
    const body = await c.req.json();
    const { id, name, categoryId, description, icon, showInMenu, onlyFolder, audience } = body || {};
    if (!id) return c.json(responses.badRequest('id required'), 400);
    const db = c.env.DB;
    const fields: string[] = [];
    const params: any[] = [];
    if (name !== undefined) { fields.push('name = ?'); params.push(String(name)); }
    if (categoryId !== undefined) { fields.push('category_id = ?'); params.push(String(categoryId)); }
    if (description !== undefined) { fields.push('description = ?'); params.push(description || ''); }
    if (onlyFolder !== undefined) { fields.push('only_folder = ?'); params.push(onlyFolder ? 1 : 0); }
    if (icon !== undefined) { fields.push('icon = ?'); params.push(icon || ''); }
    if (showInMenu !== undefined) { fields.push('show_in_menu = ?'); params.push(showInMenu ? 1 : 0); }
    let vis: string | undefined;
    if (audience && audience.visibility !== undefined) { vis = String(audience.visibility || 'public'); fields.push('audience_visibility = ?'); params.push(vis); }
    ...
    // Reset permissions
    if (audience) {
      await db.prepare('DELETE FROM category_role_permissions WHERE category_id = ?').bind(id).run();
      await db.prepare('DELETE FROM category_group_permissions WHERE category_id = ?').bind(id).run();
      if ((audience.visibility || vis) === 'restricted') {
        const allowRoles: string[] = Array.isArray(audience.allowRoles) ? audience.allowRoles : [];
        const allowGroups: string[] = Array.isArray(audience.allowGroups) ? audience.allowGroups : [];
        for (const rid of allowRoles) {
          await db.prepare(`INSERT INTO category_role_permissions (category_id, role_id) VALUES (?, ?)`).bind(id, rid).run();
        }
        for (const gid of allowGroups) {
          await db.prepare(`INSERT INTO category_group_permissions (category_id, group_id) VALUES (?, ?)`).bind(id, gid).run();
        }
      }
    }
...
categoryRoutes.delete('/', createAuthMiddleware({ required: true }), requireRole('sysadmin'), async (c) => {
  try {
    const body = await c.req.json().catch(() => ({}));
    const id = body?.id;
    if (!id) return c.json(responses.badRequest('id required'), 400);
    const res = await c.env.DB.prepare('DELETE FROM categories WHERE id = ?').bind(id).run();
    if ((res.meta?.rows_written ?? 0) === 0) return c.json(responses.notFound('Category not found'), 404);
