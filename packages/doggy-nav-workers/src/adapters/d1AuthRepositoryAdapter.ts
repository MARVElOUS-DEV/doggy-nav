...
export default class D1AuthRepositoryAdapter implements AuthRepository {
  ...
  async verifyCredentials(
    ...
    try {
      ...
      // Swallow D1 transport/parse errors and treat as invalid credentials to avoid leaking internals
      return null;
    }
  ...
  async loadAuthUser(userId: string): Promise<AuthUser> {
    ...
    const permsFromRolesRs = await this.db
      .prepare(
        `SELECT permissions FROM roles WHERE slug IN (${roleSlugs.map(() => '?').join(',') || "''"})`
      )
      .bind(...roleSlugs)
      .all<any>();
